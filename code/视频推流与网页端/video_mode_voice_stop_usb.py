...
RTSP_URL = "rtsp://49.232.181.138:8554/glasses_unified"
WEB_URL = "http://49.232.181.138:8889/glasses_unified"
...
BUTTON_POLLER = ButtonPoller({"capture": 225, "stop_video": 228})
...
STREAM_RTSP_URL = os.environ.get("STREAM_RTSP_URL", "rtsp://49.232.181.138:8554/glasses_unified")
...
def classify_command(text):
    ...
    if any(k in t for k in ["停止视频", "关闭视频", "结束视频", "停止", "关闭"]):
        return "stop_video"
...
# USB_STREAM_PROFILE_BEGIN
STREAM_SERVER_HOST = os.environ.get("STREAM_SERVER_HOST", "49.232.181.138")
STREAM_PROFILE_CONF = Path(os.environ.get("STREAM_PROFILE_CONF", "/home/elf/.config/elf_glasses_stream_profile.conf"))
WIFI_IF = os.environ.get("WIFI_IF", "p2p0")
LTE_IF = os.environ.get("LTE_IF", "wwan0")

WIFI_STREAM_CONFIG = {
    "video_caps": "video/x-raw,format=NV12,width=1280,height=720,framerate=15/1",
    "bps": 2500000, "bps_min": 2000000, "bps_max": 3000000,
    "gop": 30, "snapshot_fps": 2, "audio_bitrate": 64000,
}

LTE_STREAM_CONFIG = {
    "video_caps": "video/x-raw,format=NV12,width=640,height=360,framerate=8/1",
    "bps": 700000, "bps_min": 500000, "bps_max": 900000,
    "gop": 8, "snapshot_fps": 1, "audio_bitrate": 32000,
}

def _read_stream_profile_override():
    profile = os.environ.get("STREAM_PROFILE", "").strip().lower()
    if profile:
        return profile
    try:
        if STREAM_PROFILE_CONF.exists():
            for line in STREAM_PROFILE_CONF.read_text().splitlines():
                line = line.strip()
                if not line or line.startswith("#"):
                    continue
                if "=" in line:
                    k, v = line.split("=", 1)
                    if k.strip() == "STREAM_PROFILE":
                        return v.strip().lower()
                else:
                    return line.lower()
    except Exception as e:
        print(f"[stream-profile] read config failed: {e}", flush=True)
    return "auto"

def _route_dev_to_server():
    try:
        r = subprocess.run(
            ["ip", "route", "get", STREAM_SERVER_HOST],
        ...
        print(f"[stream-profile] route check failed: {e}", flush=True)
    return ""

def select_stream_config():
    forced = _read_stream_profile_override()
    route_dev = _route_dev_to_server()

    if forced in ("lte", "4g", "low"):
        profile = "lte"
    elif forced in ("wifi", "high", "normal"):
        profile = "wifi"
    elif route_dev == LTE_IF:
        profile = "lte"
    else:
        profile = "wifi"

    cfg = LTE_STREAM_CONFIG if profile == "lte" else WIFI_STREAM_CONFIG
    print(
        f"[stream-profile] profile={profile} forced={forced} route_dev={route_dev or 'unknown'} "
        f"caps={cfg['video_caps']} bps={cfg['bps']} audio={cfg['audio_bitrate']}",
        flush=True,
    )
    return profile, cfg
# USB_STREAM_PROFILE_END
...
def start_unified_pipeline():
    ...
    profile, stream_cfg = select_stream_config()

    cmd = [
        "gst-launch-1.0", "-q", "-e",
        "rtspclientsink", "name=sink", f"location={RTSP_URL}", "protocols=tcp",
        ...
        "!", "videorate",
        "!", "videoscale",
        "!", stream_cfg["video_caps"],
        "!", "tee", "name=video_t",

        "video_t.",
        "!", "queue",
        "!", "mpph264enc", f"bps={stream_cfg['bps']}", f"bps-min={stream_cfg['bps_min']}", f"bps-max={stream_cfg['bps_max']}",
        f"gop={stream_cfg['gop']}", "rc-mode=cbr", "profile=baseline",
        ...
        "!", "videorate",
        "!", f"video/x-raw,framerate={stream_cfg['snapshot_fps']}/1",
        "!", "videoconvert",
        "!", "jpegenc", "quality=85",
        "!", "multifilesink", f"location={LATEST_FRAME_PATH}", "max-files=1",
        "post-messages=false", "sync=false",
        ...
        "audio_t.",
        "!", "queue",
        "!", "volume", f"volume={STREAM_VOLUME}",
        "!", "opusenc", f"bitrate={stream_cfg['audio_bitrate']}", "frame-size=20",
        "!", "queue",
        "!", "sink.sink_1",

        "audio_t.",
        "!", "queue", "leaky=downstream", "max-size-buffers=30", "max-size-time=0",
        "!", "audioconvert",
        "!", "audioresample",
...
def listen_wake(audio_reader, porcupine):
    ...
    set_mode("视频通话中", "视频推流中，等待唤醒词 computer")
    print("视频推流中，继续监听唤醒词 'computer'...")
...
def handle_video_capture():
    ...
    update_status(mode="视频通话中", detail=f"已拍照，检测到：{summary}", last_capture_summary=summary)
...
def main():
    ...
    try:
        while proc.poll() is None:
            event = listen_wake(audio_reader, porcupine)

            if event == "capture":
                start_video_capture_job()
                audio_reader.flush()
                print("[VIDEO] 按钮拍照任务已转入后台，继续监听", flush=True)
                continue
